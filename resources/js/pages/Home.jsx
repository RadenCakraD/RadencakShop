...
export default function Home() {
    ...
    const { data: firstPageData, isValidating } = useSWR(`/api/products?page=1${categoryParam}`, fetcher);
    ...
    const fetchProducts = async (pageNum) => {
        try {
            setLoading(true);
            const response = await axios.get(`/api/products?page=${pageNum}${categoryParam}`);
    ...
    const filteredProducts = products.filter(p => {
        ...
        return p.nama_produk?.toLowerCase().includes(query) || (p.shop && p.shop.nama_toko?.toLowerCase().includes(query));
    });
                    ...
                    <div className="flex items-center gap-3 md:gap-5">
                        {user ? (
                            <>
                                ...
                                {/* Ikon Lonceng */}
                                <Link to="/informasi" className="text-rc-muted hover:text-rc-logo transition p-1.5 sm:p-2 relative" title="Informasi Pesanan">
                                    <i className="fa-solid fa-bell text-lg sm:text-[20px]"></i>
                                    <span className="absolute top-0 right-0 w-3 h-3 sm:w-3.5 sm:h-3.5 bg-red-600 text-white text-[7px] sm:text-[8px] font-bold flex items-center justify-center rounded-full border-[0.5px] border-rc-bg">3</span>
                                </Link>
                                    ...
                                    {isProfileOpen && (
                                        <div className="absolute right-0 top-full mt-2 w-56 bg-rc-card border-[0.5px] border-rc-main/20 rounded-md shadow-lg transition-all duration-200 origin-top-right overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-200">
                                            <div className="p-1 space-y-0.5">
                                                ...
                                                {(user.role === 'admin' || user.role === 'kurir') && <div className="h-[0.5px] w-full bg-rc-main/10 my-1"></div>}
                                                ...
                                                {(user.role === 'admin' || user.role === 'kurir') && (
                    ...
                    {[
                        ...
                        return (
                            <div key={i} onClick={() => setActiveCategory(isActive ? 'Semua' : cat.name)} className="flex-shrink-0 group cursor-pointer">
                                <div className={`w-20 h-24 md:w-28 md:h-32 rounded-[1rem] md:rounded-[1.5rem] border-[0.5px] flex flex-col items-center justify-center gap-3 md:gap-4 transition-all duration-300 relative overflow-hidden ${
                                    isActive 
                                    ...
                                    <i className={`fa-solid ${cat.icon} text-2xl md:text-3xl transition-colors duration-300 relative z-10 ${
                                        isActive ? 'text-rc-logo drop-shadow-[0_0_8px_rgba(255,204,0,0.6)]' : 'text-rc-muted group-hover:text-rc-logo/70'
                                    }`}></i>
                                    <span className={`text-[9px] md:text-[10px] font-bold text-center tracking-widest relative z-10 transition-colors uppercase px-1 ${
                                        isActive ? 'text-rc-logo' : 'text-rc-muted group-hover:text-rc-main'
                                    }`}>{cat.name}</span>
                                </div>
                            </div>
                        );
